...
def cross_map_to_img(cross_map,
                     H,
                     W,
                     question_index_list,
                     answer_index_list=[],
    ...
    """
    im = Image.new("RGB",(50*H+1,50*W+1),(0,0,0))#下地となる画像生成、マスのかずだけサイズの大きな画像になるように設定
    im2 = Image.new("RGB",(50*H+1,50*W+1),(0,0,0))#二つ目を作るのは問題用と回答用の画像２枚を出力するため
    ...
    def write_cell(x,y,string,question_index,answer_index):
        ...
        if string != "黒":#黒マスじゃない場合
            #背景色の決定
            if answer_index:
                draw.rectangle((1+(x)*50,1+(y)*50,49+(x)*50,49+(y)*50),fill=ANSWER_CELL_COLOR,outline=ANSWER_CELL_COLOR)#解答キーマスの灰色を書き込み
            else:
                draw.rectangle((1+(x)*50,1+(y)*50,49+(x)*50,49+(y)*50),fill=WHITE_CELL_COLOR,outline=WHITE_CELL_COLOR)#通常白マスを書き込み
            
            #問題のインデックスの書き込み
            if question_index:
                print("q_write x:{} y:{} w:{}".format(1+(x)*50,1+y*50,question_index))
                draw.text((1+(x)*50,1+(y)*50),question_index,fill=(0,0,0),font=font_index)
            
            #答えのインデックスの書き込み
            if answer_index:
                print("a_write x:{} y:{} w:{}".format(49+x*50,-1+y*50,answer_index))
                draw.text((49+x*50,49+y*50),answer_index,anchor="rb",fill=(0,0,0),font=font_index)#数字書き込み
                
        else:#黒マスの場合
            if question_index or answer_index:
                raise Exception("パラメータの指定に矛盾が生じています。x:{} y:{} string:{} question_index:{} answer_index:{}".format(x,y,string,question_index,answer_index))

        #解答側のマスを作成
        if string != "黒":#黒マスじゃない場合
            #背景色の決定
            if answer_index:
                draw2.rectangle((1+(x)*50,1+(y)*50,49+(x)*50,49+(y)*50),fill=ANSWER_CELL_COLOR,outline=ANSWER_CELL_COLOR)#少しサイズの小さい白い四角形の画像を上書き
            else:
                draw2.rectangle((1+(x)*50,1+(y)*50,49+(x)*50,49+(y)*50),fill=WHITE_CELL_COLOR,outline=WHITE_CELL_COLOR)#少しサイズの小さい白い四角形の画像を上書き
            draw2.text((7.5+(x)*50,(y)*50),string,fill= (0,0,0),font = font_main)#文字の書き込み
        else: #黒マスだったら何もしない
            pass
    
    for i in range(W):
        for j in range(H):
            write_cell(i,j,cross_map[j][i],question_index_map[j][i],answer_index_map[j][i])
...
def words_to_png(v_words,h_words,dict,description_save_path):
    ...
    im = Image.new("RGB",(img_width,img_height),(255,255,255))#下地となる画像生成、マスのかずだけサイズの大きな画像になるように設定
    draw = ImageDraw.Draw(im) #問題側の盤面

    line_count = 0
    draw.text((1,line_count*line_height),"縦の鍵",fill=(0,0,0),font=font_description)

    for i in v_words:
        line_count += 1
        num_string = str(i[3])+"."
        draw.text((-1+description_number_width,line_count*line_height),num_string,fill=(0,0,0),anchor="ra",font=font_description)
        draw.text((1+description_number_width,line_count*line_height),dict.get(i[2],""),fill=(0,0,0),font=font_description)
    line_count += 2
    draw.text((1,line_count*line_height),"横の鍵",fill=(0,0,0),font=font_description)
    for i in h_words:
        line_count += 1
        num_string = str(i[3])+"."
        draw.text((-1+description_number_width,line_count*line_height),num_string,fill=(0,0,0),anchor="ra",font=font_description)
...
def to_product(cross_map,
    ...
    cross_map_to_img(cross_map,H,W,question_index_list,answer_index_list,question_img_file_name,answer_img_file_name)
...
def parse_search(f_path,answer_index_character_map:str=string.ascii_uppercase):
    ...
    with open(f_path,"r",encoding="utf-8") as f:
        ...
        for _ in range(H):
            ...
            if len(h_line) != W:
                raise Exception("縦横サイズと盤面のサイズが違います H:{} W:{} h_line:{}".format(H,W,h_line))
            cross_map.append(h_line)
        answer_index_list = []
        answer_temps = f.readlines()
        for idx,i in enumerate(answer_temps):
            i = i.strip().split(",")
            answer_index_list.append((int(i[0]),int(i[1]),answer_index_character_map[idx]))
...
if __name__ == "__main__":
    ...
    path = "to_product.txt"
    product_name = "test"
    q_png_name = "img_q.png"
    a_png_name = "img_a.png"
    v_word_name = "key_v.txt"
    h_word_name = "key_h.txt"
    description_name = "description.png"
    ...
    to_product(cross_map,answer_index_list,q_save_path,a_save_path,v_word_save_path,h_word_save_path,{},description_save_path)
